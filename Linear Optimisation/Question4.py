...
def Simplex():
    ...
    C = np.array([float(val) for val in inp[0][:-1]])   # coeffs of cost function
    B = np.array([float(val) for val in [row[-1] for row in inp[1:]]])  # right hand values of coeffs
    A = np.array([[float(element) for element in row[:-1]] for row in inp[1:]])    # coeffs of constraints
    

    B = remove_degeneracy(A, B, C)  # changing into non degenerate

    Z = find_feasible_point(A, B, C)     #  finding the initial feasible point
